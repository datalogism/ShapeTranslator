"""Tests for canonical JSON -> SHACL converter.

Strategy: parse original SHACL file, convert to canonical JSON, convert back
to SHACL, then convert that SHACL back to canonical JSON and compare with the
original canonical JSON.  This roundtrip through canonical guarantees semantic
equivalence without being sensitive to serialization order.
"""
import json
import os
...
def _roundtrip_canonical(name: str) -> tuple[dict, dict]:
    """SHACL -> canonical -> SHACL -> canonical roundtrip.
    ...
    # canonical -> SHACL -> canonical
    roundtrip_shacl = convert_canonical_to_shacl(original_canonical)
    roundtrip_canonical = convert_shacl_to_canonical(roundtrip_shacl)
...
def _roundtrip_from_json_file(name: str) -> tuple[dict, dict]:
    """Load canonical JSON file -> SHACL -> canonical roundtrip.
    ...
    roundtrip_canonical = convert_shacl_to_canonical(shacl)
    roundtrip_dict = roundtrip_canonical.to_dict()
...
class TestStructure:
    """Basic structural tests for canonical -> SHACL conversion."""

    def test_produces_valid_shacl(self):
        original_shacl = parse_shacl_file(os.path.join(SHACL_DIR, "Language.ttl"))
        canonical = convert_shacl_to_canonical(original_shacl)
        shacl = convert_canonical_to_shacl(canonical)
        assert len(shacl.shapes) >= 1

    def test_target_class_preserved(self):
        original_shacl = parse_shacl_file(os.path.join(SHACL_DIR, "Language.ttl"))
        canonical = convert_shacl_to_canonical(original_shacl)
        shacl = convert_canonical_to_shacl(canonical)
        assert shacl.shapes[0].target_class.value == "http://schema.org/Language"

    def test_shape_name_preserved(self):
        original_shacl = parse_shacl_file(os.path.join(SHACL_DIR, "Language.ttl"))
        canonical = convert_shacl_to_canonical(original_shacl)
        shacl = convert_canonical_to_shacl(canonical)
        assert "Language" in shacl.shapes[0].iri.value

    def test_serializable(self):
        original_shacl = parse_shacl_file(os.path.join(SHACL_DIR, "Event.ttl"))
        canonical = convert_shacl_to_canonical(original_shacl)
        shacl = convert_canonical_to_shacl(canonical)
        output = serialize_shacl(shacl)
        assert "@prefix sh:" in output
        assert "sh:NodeShape" in output

    def test_property_count_preserved(self):
...
class TestCanonicalRoundtrip:
    """SHACL -> canonical -> SHACL -> canonical must produce identical canonical."""
    ...
    def test_taxon_roundtrip(self):
        original, roundtrip = _roundtrip_canonical("Taxon")
        assert original == roundtrip, _diff_report("Taxon", original, roundtrip)
...
class TestBatch:
    """Ensure all 37 YAGO files roundtrip through canonical -> SHACL -> canonical."""
