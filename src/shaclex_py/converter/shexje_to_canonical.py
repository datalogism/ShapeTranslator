...
def convert_shexje_to_canonical(schema: ShexJESchema) -> CanonicalSchema:
    ...
    # Place the start shape first so name-based lookups find it before any
    # value-shape stubs that may share the same CamelCase ID.
    start_decl = shape_map.get(schema.start) if schema.start else None
    if start_decl is not None:
        cs = _convert_decl(start_decl, shape_map)
        if cs is not None:
            shapes.append(cs)
...
def _is_rdf_type_stub(shape: ShapeE) -> bool:
    """Return True for shapes whose sole property is an unconstrained rdf:type.

    These are value-shape stubs emitted by LLMs that carry no useful schema
    information and should not appear as top-level canonical shapes.
    """
...
def _convert_decl(
    ...
    if isinstance(decl, ShapeE):
        ...
        if decl.predicate is not None and decl.targetClass is None:
            return None
        # Skip full-expression stubs: no targetClass + only an unconstrained
        # rdf:type triple (or empty expression). These are generated by LLMs as
        # auxiliary value-shape stubs and pollute the canonical shape list.
        if decl.targetClass is None and _is_rdf_type_stub(decl):
            return None
